...
// Резервный приёмник заявок на случай, когда LEAD_BOT_TOKEN/LEAD_CHAT_ID
// не заданы или Telegram отказал. Переопределяется переменной LEAD_RELAY_URL.
const RELAY_URL = process.env.LEAD_RELAY_URL || 'https://repid-leads.vercel.app/api/lead';
...
export async function POST(req: NextRequest) {
  ...
  // Резервный путь. Пока переменные LEAD_BOT_TOKEN/LEAD_CHAT_ID на проде не
  // заданы, прямая отправка невозможна и заявки теряются — поэтому заявка
  // уходит на внешний приёмник, где токен бота хранится в его собственных
  // переменных окружения. Как только переменные появятся здесь, этот путь
  // перестанет использоваться сам собой.
  try {
    ...
    if (relay.ok) return NextResponse.json({ ok: true, via: 'relay' });
    lastError = 'relay http ' + relay.status;
  } catch {
    lastError = 'relay unreachable';
  }

  // Ни один путь не сработал — заявка остаётся хотя бы в логах Vercel.
